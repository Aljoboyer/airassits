...
function App() {
  ...
  const getHumeAccessToken = async (apiKey, secretKey) => {
    const response = await fetch('http://localhost:3001/hume-token', {
        method: 'GET',
      });
  ...
  ws.onopen = () => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        console.log("📥 Hume EVI Response:", response);
        ...
        if (response.type === "assistant_message") {
          console.log("🧠 EVI says:", response.content);
        }
      } catch (err) {
